...
export default function App() {
  ...
  const [todos, setTodos] = React.useState(
    () => JSON.parse(localStorage.getItem("teejayMyTodos")) || []
  );
  ...
  const [currQuote, setCurrQuote] = React.useState(
    () => quotes[Math.floor(Math.random() * quotes.length)]
  );
  const [showQuote, setShowQuote] = React.useState(true);
  const [darkMode, setDarkMode] = React.useState(() => {
    return localStorage.getItem("teejayMyTodosDarkMode")
      ? JSON.parse(localStorage.getItem("teejayMyTodosDarkMode"))
      : window.matchMedia &&
  ...
  function changeCurrTodo(id) {
    ...
    setTodos((oldTodos) => {
      ...
      for (let i = 0; i < oldTodos.length; i++) {
        oldTodos[i].id === id
          ? updateTodos.unshift(oldTodos[i])
          : updateTodos.push(oldTodos[i]);
      }
      return updateTodos;
    });
    ...
    if (newTodoItem !== "") {
      const newTodos = todos.map((todo) => {
        if (todo.id === id) {
          return {
            ...todo,
            todos: [{ title: newTodoItem, completed: false }, ...todo.todos],
          };
        } else {
          return todo;
        }
  ...
  function addNewTodo() {
    const newTodos = [
      { id: nanoid(), title: "New To-Do List (Change Subject)", todos: [] },
      ...todos,
    ];
    setTodos(newTodos);
    setCurrTodos(newTodos[0]);
  ...
  React.useEffect(() => {
    ...
    todos.length > 0 &&
      document.getElementById("title" + currTodos.id).classList.add("active");
  }, [currTodos, todos]);
  ...
  const titleTodos =
    todos.length > 0 &&
    todos.map((todo) => (
      <div key={`title${todo.id}`} onClick={() => changeCurrTodo(todo.id)}>
        <li id={`title${todo.id}`} className="todoList">
          <span>
            <div className={checkCompleted(todo) ? "fas fa-check" : ""}>
              {" "}
  ...
  return (
    <div className={darkMode ? "App darkMode" : "App"}>
      ...
      <div className="quotes">
        <i
          style={{
            color: showQuote ? (darkMode ? "white" : "black") : "transparent",
          }}
        >
          {currQuote}
        </i>
      ...
      {todos.length > 0 ? (
        ...
        <main className="no-todos">
          ...
          <button className="first-todo" onClick={firstNewTodo}>
            Create New To-Do
          </button>
        </main>
      )}
